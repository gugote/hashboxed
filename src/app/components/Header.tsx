...
export default function Header({ layoutClass, caseStudy }: HeaderProps){
  ...
  return (
    <header className={`${layoutClass}`}>
      ...
      {caseStudy && (
        <div className="caseStudyTag py-2 px-4 border border-dashed border-slate-400 bg-white rounded-md">
          <p className="text-xs uppercase"><strong>CLIENT:</strong> {caseStudy.client} / <strong>DATE:</strong> {caseStudy.date}</p>
        </div>
      )}
    </header>
  )
