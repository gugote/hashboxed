...
export default function Header({ isSpinning, layoutClass }: HeaderProps){
  ...
  const noDarkMode = usePathname().includes('works') ? false : true;
  ...
  return (
    <header className={`${layoutClass} relative flex flex-row align-middle`}>
      ...
      <Navigation layoutClass="absolute top-[22px] right-0" />
      {noDarkMode &&(
        <ToggleTheme layoutClass="absolute top-[22px] right-10"/>
      )}
    </header>
  )
}